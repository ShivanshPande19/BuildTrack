...
> **Both edits above are applied and committed.** `ios/` and `android/` are tracked
> in this repo, so a fresh clone already has them. They are shown here so the
> scheme can be traced back to `inviteRedirectUrl` in
> `lib/core/supabase_client.dart` and to the Supabase Redirect URL setting — all
> three have to say `io.supabase.buildtrack` or the invite link dead-ends.
>
> If you ever regenerate the platform folders, these two are the first things to
> re-apply: `flutter create .` overwrites both files with stock templates, and the
